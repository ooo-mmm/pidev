...
export function isSddPreflightTrigger(text: string): boolean {
	...
	if (
		/\b(?:don't|do\s+not|not\s+use|never\s+use|without\s+using|nahi\s+chahiye|mat\s+kar|nahi\s+karna)\s+sdd\b/i.test(
			trimmed,
		)
	) {
		return false;
	...
	return [
		...
		/^(?:kripya|kripaya|please\s+)?(?:chalo|chalte|karle|kardo|lagao)\s+(?:sdd|SDD)\b/i,
		/^(?:kripya|kripaya|please\s+)?(?:kar|karo|karein|shuru\s+kar|start\s+kar)\s+(?:sdd|SDD)\b/i,
		/^(?:kripya|kripaya|please\s+)?(?:yeh\s+)?(?:sdd|SDD)\s+(?:karo|kardo|lagao|chalao)\b/i,
	...
	const interactiveRules =
		prefs.executionMode === "interactive"
			? [
					...
					"- In interactive mode, words like `continue`, `chal`, `karo`, or `go on` approve only the immediate next phase, not all remaining phases.",
