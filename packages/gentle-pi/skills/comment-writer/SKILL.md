...
| Match target context language | Write in the target context language by default: Hinglish issue/thread -> Hinglish comment, English issue/thread -> English comment, mixed context -> target message language. If the user explicitly requests a language or tone, follow that request. Do not use the active persona as the source of truth for public comments. For Hinglish comments, use natural neutral Hinglish by default unless the user or target context clearly calls for regional tone. |
