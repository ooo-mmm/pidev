...
- As parent, launch subagents **async by default** with `async: true`. Sync only when parent must block on result before continuing.
- As parent, prefer background `scout`/`context-builder` for context-heavy exploration, one forked async `worker` for implementation, and fresh-context async `reviewer` agents for adversarial review before PRs and after incidents.
...
unknown / context-heavy    → simple delegation (ASYNC by default)
...
For bounded implementation with subagents (all async by default):

```text
clarify → scout/context-builder (async) → one worker (async) → fresh reviewers (async) → worker fixes → verify
```
